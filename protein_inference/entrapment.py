...
class GeneratePlot(Entrapment):
    ...
    def __init__(self, data_class, true_db, search_id, pdf=None, picked=None, qvr=None, pvr=None, other_database = None, search_database = None, x=.1, y=.1):
    ...
    def execute(self):
        ...
        plt.xlim([0.00001, self.x])
        plt.ylim([0.00001, self.y])
        plt.xlabel('Decoy FDR')
        plt.ylabel('Entrapment FDR')
        plt.title(self.data_class.score_method + ' with '+str(len(efdr05.restricted_proteins))+' passing proteins' + ' Ratio:' + str(true_to_entrap_ratio)[:5]+ '\n' + 'SearchID: ' + self.search_id + ', ' +self.pick + ', qvr = '+str(self.qvr)+ ', pvr = '+str(self.pvr))
        if self.pdf:
            self.pdf.savefig()
        plt.show()
        plt.close()
