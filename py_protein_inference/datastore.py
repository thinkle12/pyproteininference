...
class DataStore(object):
    ...
    Attributes:
        ...
        parameter_file_object (py_protein_inference.parameters.ProteinInferenceParameter): protein inference parameter object
        restricted_peptides (list): List of non flaking peptide strings present in the current analysis
        main_data_restricted (list): List of restricted :py:class:`py_protein_inference.physical.Psm` objects. Restriction is based on the parameter_file_object and the object is created by function :py:meth:`py_protein_inference.datastore.DataStore.restrict_psm_data`
        scored_proteins (list): List of scored :py:class:`py_protein_inference.physical.Protein` objects. Output from scoring methods from :py:mod:`py_protein_inference.scoring`
        grouped_scored_proteins (list): List of scored :py:class:`py_protein_inference.physical.Protein` objects that have been grouped and sorted. Output from :py:meth:`py_protein_inference.inference.Inference.run_inference` method
        scoring_input (list): List of non-scored :py:class:`py_protein_inference.physical.Protein` objects. Output from :py:meth:`py_protein_inference.datastore.DataStore.create_scoring_input`
        picked_proteins_scored (list): List of :py:class:`py_protein_inference.physical.Protein` objects that pass the protein picker algorithm (:py:meth:`py_protein_inference.datastore.DataStore.protein_picker`)
        picked_proteins_removed (list): List of :py:class:`py_protein_inference.physical.Protein` objects that do not pass the protein picker algorithm (:py:meth:`py_protein_inference.datastore.DataStore.protein_picker`)
        ...
        psm_score (str): Variable that indicates the :py:class:`py_protein_inference.physical.Psm` score being used in the analysis to generate :py:class:`py_protein_inference.physical.Protein` scores
    ...
    def get_sorted_identifiers(self, scored=True):
        ...
        our_decoy_sp_proteins = sorted(
            [x for x in proteins if x in all_sp_proteins and self.decoy_symbol in x]
        )
        ...
        our_target_tr_proteins = sorted(
            [
                x
                for x in proteins
                if x not in all_sp_proteins and self.decoy_symbol not in x
            ]
        )
        our_decoy_tr_proteins = sorted(
            [x for x in proteins if x not in all_sp_proteins and self.decoy_symbol in x]
        )

        our_proteins_sorted = (
            our_target_sp_proteins
            + our_decoy_sp_proteins
            + our_target_tr_proteins
            + our_decoy_tr_proteins
        )
    ...
    def get_protein_data(self):
        ...
        Retrieves picked and scored data if the data has been picked and scored or just the scored data if the data has not been picked.
    ...
    def restrict_psm_data(self, remove1pep=True):
        ...
        Restriction values are pulled from the :py:class:`py_protein_inference.parameters.ProteinInferenceParameter` object
        ...
        if peptide_length and posterior_error_prob_threshold and not q_value_threshold:
            ...
            for psms in main_psm_data:
                if len(
                    psms.stripped_peptide
                ) >= peptide_length and psms.pepvalue < float(
        ...
        if (
            peptide_length
            and not posterior_error_prob_threshold
            and not q_value_threshold
        ):
            restricted_data = []
            for psms in main_psm_data:
        ...
        if (
            not peptide_length
            and posterior_error_prob_threshold
            and not q_value_threshold
        ):
            restricted_data = []
            for psms in main_psm_data:
    ...
    def create_scoring_input(self):
        ...
        This method initializes a list of :py:class:`py_protein_inference.physical.Protein` objects to get them ready to be scored by :py:mod:`py_protein_inference.scoring.Score` methods
        ...
        logger = getLogger(
            "py_protein_inference.datastore.DataStore.create_scoring_input"
        )
        ...
        if self.parameter_file_object.inference_type != Inference.PEPTIDE_CENTRIC:
            ...
            for psms in psm_data:
                ...
                sorted_protein_list = sorted_protein_list[
                    : self.parameter_file_object.max_identifiers_peptide_centric
                ]
    ...
    def protein_to_peptide_dictionary(self):
        """
        Method that returns a map of protein strings to sets of peptide strings and is essentially half of a BiPartite graph
        This method sets the :attr:`protein_peptide_dictionary` Attribute for the :py:class:`py_protein_inference.datastore.DataStore` object
    ...
    def peptide_to_protein_dictionary(self):
        """
        Method that returns a map of peptide strings to sets of protein strings and is essentially half of a BiPartite graph
        This method sets the :attr:`peptide_protein_dictionary` Attribute for the :py:class:`py_protein_inference.datastore.DataStore` object
    ...
    def unique_to_leads_peptides(self):
        ...
        if self.grouped_scored_proteins:
            ...
            unique_to_leads_peptides = set(
                [x for x in counted_peps if counted_peps[x] == 1]
            )
        else:
            unique_to_leads_peptides = set()
    ...
    def exclude_non_distinguishing_peptides(
        ...
        The method sets the :attr:`scoring_input` and :attr:`restricted_peptides` variables for the :py:class:`py_protein_inference.datastore.DataStore` object
        ...
        logger = getLogger(
            "py_protein_inference.datastore.DataStore.exclude_non_distinguishing_peptides"
        )
        ...
        raw_peps = [
            x.raw_peptides
            for x in self.scoring_input
            if x.identifier in non_subset_proteins
        ]

        # Make the raw peptides a flat list
        flat_peptides = [
            Psm.split_peptide(peptide_string=item)
            for sublist in raw_peps
            for item in sublist
        ]
        ...
        self.restricted_peptides = [
            x for x in self.restricted_peptides if x in new_flat_peptides
        ]
    ...
    def protein_picker(self):
        ...
        In the Target/Decoy pair the protein with the better score is kept and the one with the worse score is discarded from the analysis

        The method sets the :attr:`picked_proteins_scored` and :attr:`picked_proteins_removed` variables for the :py:class:`py_protein_inference.datastore.DataStore` object
        ...
        decoy_proteins = [
            x.identifier
            for x in self.scored_proteins
            if self.decoy_symbol in x.identifier
        ]
    ...
    def calculate_q_values(self, regular=True):
        ...
        This method updates the :attr:`protein_group_objects` for the :py:class:`py_protein_inference.datastore.DataStore` object by updating the q_value variable of the :py:class:`py_protein_inference.physical.ProteinGroup` objects
    ...
    def entrapment_fdr(self, true_database, false_discovery_rate=0.05):
        ...
        for i in range(len(protein_data)):
            ...
            if (
                not protein_data[i].startswith(self.decoy_symbol)
                and protein_data[i] not in true_proteins
            ):
        ...
        fdr_list = []
        for i in range(len(lead_proteins)):
            binary_entrap_target_list = [
                1 if elem in entrapment_proteins_set else 0 for elem in lead_proteins
            ]
            total = len(lead_proteins)
    ...
    def _check_data_digest_overlap_psms(self):
        ...
        """
        logger = getLogger(
            "py_protein_inference.datastore.DataStore._check_data_digest_overlap_psms"
        )
        ...
        percentage = float(len(set(peptides))) / float(
            len(set(peptides_from_search_in_digest))
        )
        ...
        logger.info(
            "{}; ratio of PSMs identified from input files to those that are present in the search and in the database digestion".format(
                percentage
            )
        )
    ...
    def _check_data_digest_overlap_proteins(self):
        ...
        """
        logger = getLogger(
            "py_protein_inference.datastore.DataStore._check_data_digest_overlap_proteins"
        )
        ...
        percentage = float(len(flat_proteins)) / float(
            len(proteins_from_search_in_digest)
        )
        logger.info(
            "{} proteins identified from input files".format(len(flat_proteins))
        )
        logger.info(
    ...
    def _check_target_decoy_split(self):
        ...
        """
        logger = getLogger(
            "py_protein_inference.datastore.DataStore._check_target_decoy_split"
        )
    ...
    def _validate_decoys_from_data(self):
        ...
        """
        logger = getLogger(
            "py_protein_inference.datastore.DataStore._validate_decoys_from_data"
        )
        ...
        flat_proteins = set([item for sublist in proteins for item in sublist])
        targets = [
            x for x in flat_proteins if self.parameter_file_object.decoy_symbol not in x
        ]
        decoys = [
            x for x in flat_proteins if self.parameter_file_object.decoy_symbol in x
        ]
    ...
    def _validate_isoform_from_data(self):
        ...
        """
        logger = getLogger(
            "py_protein_inference.datastore.DataStore._validate_isoform_from_data"
        )
    ...
    def _validate_reviewed_v_unreviewed(self):
        ...
        """
        logger = getLogger(
            "py_protein_inference.datastore.DataStore._validate_reviewed_v_unreviewed"
        )
        ...
        logger.info(
            "Number of Reviewed Proteins in from Digest: {}".format(reviewed_proteins)
        )
        logger.info(
            "Number of Unreviewed Proteins in from Digest: {}".format(
                unreviewed_proteins
            )
        )
    ...
    def sort_protein_strings(cls, protein_string_list, sp_proteins, decoy_symbol):
        ...
        our_target_sp_proteins = sorted(
            [
                x
                for x in protein_string_list
                if x in sp_proteins and decoy_symbol not in x
            ]
        )
        our_decoy_sp_proteins = sorted(
            [x for x in protein_string_list if x in sp_proteins and decoy_symbol in x]
        )
        ...
        our_target_tr_proteins = sorted(
            [
                x
                for x in protein_string_list
                if x not in sp_proteins and decoy_symbol not in x
            ]
        )
        our_decoy_tr_proteins = sorted(
            [
                x
                for x in protein_string_list
                if x not in sp_proteins and decoy_symbol in x
            ]
        )
    ...
    def input_has_q(self):
        ...
        if len_q == len_all:
            ...
            status = False
            self.logger.warning(
                "Input does not have Q value; Cannot restrict by Q value"
            )
    ...
    def input_has_pep(self):
        ...
        if len_pep == len_all:
            ...
            status = False
            self.logger.warning(
                "Input does not have Pep value; Cannot restrict by Pep value"
            )
    ...
    def input_has_custom(self):
        ...
        if len_c == len_all:
            ...
            status = False
            self.logger.warning(
                "Input does not have Custom value; Cannot restrict by Custom value"
            )
