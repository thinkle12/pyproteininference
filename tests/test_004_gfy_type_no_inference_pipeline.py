...
PARAMETER_FILE = resource_filename(
    "protein_inference", "../tests/data/test_params_no_inference.yaml"
)
...
LEAD_OUTPUT_FILE = resource_filename(
    "protein_inference",
    "../tests/output/leads/test_no_inference_q_value_leads_ml_posterior_error_prob.csv",
)
ALL_OUTPUT_FILE = resource_filename(
    "protein_inference",
    "../tests/output/all/test_no_inference_q_value_all_ml_posterior_error_prob.csv",
)
PEPTIDE_OUTPUT_FILE = resource_filename(
    "protein_inference",
    "../tests/output/peptides/test_no_inference_q_value_leads_peptides_ml_posterior_error_prob.csv",
)
PSM_OUTPUT_FILE = resource_filename(
    "protein_inference",
    "../tests/output/psms/test_no_inference_q_value_leads_psms_ml_posterior_error_prob.csv",
)
PSM_ID_OUTPUT_FILE = resource_filename(
    "protein_inference",
    "../tests/output/psm_ids/test_no_inference_q_value_leads_psm_ids_ml_posterior_error_prob.csv",
)
...
logger = logging.getLogger(
    "protein_inference.tests.test_004_gfy_type_no_inference_pipeline.py"
)
...
class TestLoadNoInferenceWorkflow(TestCase):
    def test_workflow_no_inference(self):
        ...
        if inference_type == protein_inference.inference.Inference.FIRST_PROTEIN:
            group = protein_inference.inference.FirstProtein(
                data_class=data, digest_class=digest
            )
            group.infer_proteins()
